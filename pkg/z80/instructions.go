...
func addb(r *Registers, a, b uint8, cy bool) uint8 {
	...
	if carry(4, uint16(a), uint16(b), cy) {
		r.F |= FlagH
	} // half-carry: bit 3→4
	if carry(7, uint16(a), uint16(b), cy) != carry(8, uint16(a), uint16(b), cy) {
		r.F |= FlagP
	}
	if carry(8, uint16(a), uint16(b), cy) {
		r.F |= FlagC
	}
...
func (c *CPU) execOpcodeCB(opcode uint8) int {
	...
	// Cycle counts: (HL) operands are slower than register operands.
	cycles := 8
	switch x {
	case 0: // Rotate/shift
		val := c.getCBResult(z)
...
func (c *CPU) execOpcodeDCB(opcode uint8, addr uint16) int {
	...
	// Displaced operations cost 23 T-states, except BIT which is 20.
	cycles := 23
	switch x {
...
func (c *CPU) execOpcodeED(opcode uint8) int {
	...
	// --- IN r,(C) : ED 40,48,50,58,60,68,78 (ED 70 = IN (C), flags only) ---
	//
	// The destination register is selected by bits 3-5 of the opcode; the low
	// three bits only separate IN from OUT. Selecting the register with the low
	// bits picks B for every one of these opcodes, so IN A,(C) reads the port and
	// then throws the value away into B. The Spectrum ROM keeps the keyboard row
	// selector in B during its scan, so that silently breaks the whole keyboard.
	case opcode == 0x40 || opcode == 0x48 || opcode == 0x50 || opcode == 0x58 ||
		opcode == 0x60 || opcode == 0x68 || opcode == 0x70 || opcode == 0x78:
		port := (uint16(r.B) << 8) | uint16(r.C)
		val := c.readIOPort(port)
		if opcode != 0x70 {
			c.setCBResult((opcode>>3)&7, val)
		}
		r.F &^= FlagS | FlagZ | FlagY | FlagX | FlagH | FlagP | FlagN
		if val&0x80 != 0 {
			r.F |= FlagS
	...
	case opcode == 0x41 || opcode == 0x49 || opcode == 0x51 || opcode == 0x59 ||
		opcode == 0x61 || opcode == 0x69 || opcode == 0x71 || opcode == 0x79:
		port := (uint16(r.B) << 8) | uint16(r.C)
		var val uint8
		if opcode != 0x71 {
			val = c.getCBResult((opcode >> 3) & 7)
		}
		c.writeIOPort(port, val)
		return 12
	}
...
func (c *CPU) execOpcodeDDFD(opcode uint8, izh, izl *uint8) int {
	...
	case 0x46, 0x4E, 0x56, 0x5E, 0x66, 0x6E, 0x7E, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x77:
		addr := izAddr()
		// The destination field selects the direction: 6 means (IZ+d).
		// Classifying by the source field here made LD r,(IZ+d) behave as a
		// store, which wrote a stray byte (read from (HL)) over the operand
		// address and left the register unloaded.
		dst := (opcode >> 3) & 7
		if dst == 6 {
			// LD (IZ+d), r
		...
		// LD r, (IZ+d), including 0x7E (LD A, (IZ+d))
		c.setCBResult(dst, c.rb(addr))
		return 19

	// --- 8-bit arithmetic on (IZ+d) ---
	case 0x86, 0x8E, 0x96, 0x9E, 0xA6, 0xAE, 0xB6, 0xBE:
		addr := izAddr()
		// Delegate to the base opcode with (HL) semantics; the helper reads the
		// operand itself, so the operation must not be applied here as well.
		c.execOpcodeBaseWithHL(opcode, addr)
		return 19
...
func (c *CPU) execOpcodeBase(opcode uint8) int {
	...
	case opcode == 0x02:
		c.wb(r.BC(), r.A)
		r.WZ = (r.BC() & 0xFF00) | ((r.BC() + 1) & 0xFF)
		return 7
	case opcode == 0x12:
		c.wb(r.DE(), r.A)
		r.WZ = (r.DE() & 0xFF00) | ((r.DE() + 1) & 0xFF)
		return 7
	case opcode == 0x0A:
		r.A = c.rb(r.BC())
		r.WZ = r.BC() + 1
	...
	case opcode >= 0x40 && opcode <= 0x7F:
		src := opcode & 7
		dst := (opcode >> 3) & 7
		c.setCBResult(dst, c.getCBResult(src))
		if src == 6 || dst == 6 {
			return 7 // one operand is (HL)
		}
		return 4

	// === 0x80–0xBF: ALU ops (ADD/ADC/SUB/SBC/AND/XOR/OR/CP) A, r ===
	case opcode >= 0x80 && opcode <= 0xBF:
		val := c.getCBResult(opcode & 7)
		switch (opcode >> 3) & 7 {
		case 0: // ADD A, r
			r.A = addb(r, r.A, val, false)
		case 1: // ADC A, r
		...
		if opcode&7 == 6 {
			return 7 // source is (HL)
		}
		return 4
	case opcode == 0xC6:
		r.A = addb(r, r.A, c.nextb(), false)
		return 7 // ADD A, n
	case opcode == 0xCE:
		r.A = addb(r, r.A, c.nextb(), r.F&FlagC != 0)
		return 7 // ADC A, n
	case opcode == 0xD6:
		r.A = subb(r, r.A, c.nextb(), false)
		return 7 // SUB n
	case opcode == 0xDE:
		r.A = subb(r, r.A, c.nextb(), r.F&FlagC != 0)
		return 7 // SBC A, n
	case opcode == 0xE6:
		land(r, c.nextb())
		return 7 // AND n
...
func (c *CPU) Step() int {
	...
	// HALT: the CPU repeats internal NOPs until an interrupt is accepted. The
	// interrupt sequence is charged to this step so that the frame's T-state
	// budget stays accurate. PC is left pointing at the instruction after HALT
	// (it advanced when HALT was fetched), so the pushed return address resumes
	// there once the handler returns.
	if r.Halted {
		cycles := 4
...
// acceptInterrupt performs the maskable interrupt acknowledge sequence and
// returns the T-states it costs. It is shared by the normal instruction path and
// by the HALT state. Maskable interrupts are disabled for the duration of the
// handler (IFF1 = IFF2 = 0), as on real hardware.
func (c *CPU) acceptInterrupt() int {
	...
	case 0:
		...
		return 13
	case 2:
		// IM 2: indirect call through the vector table.
